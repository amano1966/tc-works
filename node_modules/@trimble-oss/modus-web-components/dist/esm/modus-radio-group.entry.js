...
const ModusRadioButton = (props) => (h("div", { class: `modus-radio-button ${props.disabled ? 'disabled' : ''}` },
    h("div", { class: `radio ${props.size}`, tabIndex: 0, ref: props.ref, onClick: () => props.handleButtonClick(props.id), onKeyDown: (event) => props.handleKeydown(event, props.id) },
...
const ModusRadioGroup = class {
    ...
    render() {
        return (h("div", { key: 'f739d67545febe34e1b7f1895aafa8b640d8a23f', class: "modus-radio-group", "aria-label": this.ariaLabel }, this.radioButtons.map((radioButton, index) => {
            return (h(ModusRadioButton, { ref: (el) => (this.radioButtonRefs[index] = el), size: this.size, checked: radioButton.checked, disabled: radioButton.disabled, label: radioButton.label, name: this.name, id: radioButton.id, handleButtonClick: (id) => this.handleButtonClick(id), handleKeydown: (event, id) => this.handleButtonKeydown(event, id) }));
        })));
    }
