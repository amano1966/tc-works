...
const ModusPagination = class {
    ...
    renderPreviousPageControl() {
        return (this.maxPage - this.minPage >= 7 && (index.h("li", null, index.h("button", { "aria-label": "Previous", class: `${this.activePage != this.minPage ? 'hoverable' : 'disabled'}`, disabled: this.activePage === this.minPage, onClick: () => this.handleChevronClick(PaginationDirection.Previous), onKeyDown: (event) => this.handleChevronKeydown(event, PaginationDirection.Previous) }, this.prevPageButtonText ? (index.h("span", { "data-test-id": "prev-button-text" }, this.prevPageButtonText)) : (index.h(IconChevronLeft.IconChevronLeft, { size: this.chevronSizeBySize.get(this.size) }))))));
    }
    renderNextPageControl() {
        return (this.maxPage - this.minPage >= 7 && (index.h("li", null, index.h("button", { "aria-label": "Next", class: `${this.activePage != this.maxPage ? 'hoverable' : 'disabled'}`, disabled: this.activePage === this.maxPage, onClick: () => this.handleChevronClick(PaginationDirection.Next), onKeyDown: (event) => this.handleChevronKeydown(event, PaginationDirection.Next) }, this.nextPageButtonText ? (index.h("span", { "data-test-id": "next-button-text" }, this.nextPageButtonText)) : (index.h(IconChevronRight.IconChevronRight, { size: this.chevronSizeBySize.get(this.size) }))))));
    }
    renderPageNumbers() {
        return this.pages.map((page) => {
            const isCurrentPage = page === this.activePage;
            if (page === '...') {
                return (index.h("li", null, index.h("button", { class: `${!isNaN(+page) ? 'hoverable' : ''}`, tabIndex: -1 }, page)));
            }
            return (index.h("li", null, index.h("button", { "aria-current": isCurrentPage ? 'page' : null, class: `${page === this.activePage ? 'active' : ''} ${!isNaN(+page) ? 'hoverable' : ''}`, onClick: () => this.handlePageClick(+page), onKeyDown: (event) => this.handlePageKeydown(event, +page) }, page)));
        });
    }
    render() {
        return (index.h("nav", { key: '3daf4148138b3f2532d1fc5398a5166ba6d0af23', "aria-label": this.ariaLabel || undefined, class: `${this.classBySize.get(this.size)}` }, index.h("ul", { key: 'd129a1b45d58ab78102a0a55143c3fd22cae7484' }, this.renderPreviousPageControl(), this.renderPageNumbers(), this.renderNextPageControl())));
    }
...
const ModusTableCellLinkElement = ({ link, onLinkClick }) => {
    function handleLinkKeyDown(e) {
        ...
        if (key === modusTable_constants.KEYBOARD_ENTER || key === modusTable_constants.KEYBOARD_SPACE) {
            onLinkClick(link);
            e.stopImmediatePropagation();
        }
    }
    return (index.h("div", { class: "cell-link", tabIndex: 0, onClick: () => onLinkClick(link), onKeyDown: handleLinkKeyDown }, link.display));
};
...
const ModusTableCellBadgeElement = ({ badge, onBadgeClick }) => {
    ...
    return (index.h("div", { class: "cell-badge", onClick: () => onBadgeClick(badge), onKeyDown: () => handleBadgeKeyDown },
        index.h("modus-badge", { tabIndex: 0, color: badge.color, size: badge.size, type: badge.type, ariaLabel: badge === null || badge === void 0 ? void 0 : badge.ariaLabel }, badge.text)));
};

// eslint-disable-next-line
const IconChevronDownThick = (props) => {
    var _a, _b, _c;
    return (index.h("svg", { class: `icon-chevron-down-thick ${props.pressed ? 'pressed' : ''}`, fill: (_a = props.color) !== null && _a !== void 0 ? _a : 'currentColor', height: (_b = props.size) !== null && _b !== void 0 ? _b : 16, width: (_c = props.size) !== null && _c !== void 0 ? _c : 16, onClick: props.onClick, viewBox: "0 0 24 24" },
...
const IconChevronUpThick = (props) => {
    var _a, _b, _c;
    return (index.h("svg", { class: `icon-chevron-up-thick ${props.pressed ? 'pressed' : ''}`, fill: (_a = props.color) !== null && _a !== void 0 ? _a : 'currentColor', height: (_b = props.size) !== null && _b !== void 0 ? _b : 16, width: (_c = props.size) !== null && _c !== void 0 ? _c : 16, onClick: props.onClick, viewBox: "0 0 24 24" },
...
const ModusTableCellExpandIcons = ({ row }) => {
    let expandEl;
    return (index.h("span", { class: "expand-icon-container", ref: (ref) => (expandEl = ref), style: { paddingLeft: `${row.depth * 2}rem` }, onClick: (e) => {
            row.getToggleExpandedHandler()();
            e.stopImmediatePropagation();
        } }, row.getCanExpand() && (index.h("span", { class: "expand-icon", tabIndex: 0, onKeyDown: (event) => {
            if (event.key.toLowerCase() === modusTable_constants.KEYBOARD_ENTER || event.key.toLowerCase() === modusTable_constants.KEYBOARD_SPACE) {
                expandEl.click();
                event.stopImmediatePropagation();
            }
        } }, row.getIsExpanded() ? index.h(IconChevronUpThick, { size: '24' }) : index.h(IconChevronDownThick, { size: '24' })))));
};

const ModusTableCustomCellElement = ({ customElement }) => {
    return index.h("div", { class: "cell-custom" },
        " ",
        index.h("flex-renderer", { content: customElement.getValue().toString() }));
};

const ModusTableCellMain = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.cellInputValueChange = index.createEvent(this, "cellInputValueChange", 7);
        ...
        this.handleCellEditorKeyDown = (event, newValue, oldValue) => {
            ...
            if (key === modusTable_constants.KEYBOARD_ENTER) {
                this.handleCellEditorValueChange(newValue, oldValue);
                tableCellNavigation_utility.NavigateTableCells({
                    eventKey: modusTable_constants.KEYBOARD_ENTER,
                    cellElement: this.cellEl,
                });
            }
            else if (key === modusTable_constants.KEYBOARD_ESCAPE) {
                this.editMode = false;
                this.cellEl.focus();
    ...
    getEditorType() {
        const editorType = this.cell.column.columnDef[modusTable_constants.COLUMN_DEF_CELL_EDITOR_TYPE_KEY];
        const dataType = this.cell.column.columnDef[modusTable_constants.COLUMN_DEF_DATATYPE_KEY];
        let editorTypeToReturn = modusTable_constants.CELL_EDIT_TYPE_TEXT;
        if (!editorType) {
            if (modusTable_constants.ALLOWED_CELL_EDIT_TYPES.includes(dataType))
    ...
    getEditorArgs() {
        return this.cell.column.columnDef[modusTable_constants.COLUMN_DEF_CELL_EDITOR_ARGS_KEY];
    }
    /**
    ...
    renderCellValue() {
        ...
        const cellDataType = (_a = cellValue['_type']) !== null && _a !== void 0 ? _a : this.cell.column.columnDef[modusTable_constants.COLUMN_DEF_DATATYPE_KEY];
        const wrap = cellDataType === modusTable_constants.COLUMN_DEF_DATATYPE_BADGE ? false : wrapText;
        const classes = {
            'cell-content': true,
            'truncate-text': !wrap,
            'wrap-text': wrap,
            'text-align-right': cellDataType === modusTable_constants.COLUMN_DEF_DATATYPE_INTEGER,
        };
        const renderCell = () => {
            if (cellDataType === modusTable_constants.COLUMN_DEF_DATATYPE_CUSTOM) {
                return index.h(ModusTableCustomCellElement, { customElement: cell });
            }
            else if (cellDataType === modusTable_constants.COLUMN_DEF_DATATYPE_LINK) {
                return (index.h(ModusTableCellLinkElement, { link: cellValue, onLinkClick: (link) => {
                        this.cellEl.focus();
                        cellLinkClick.emit(link);
                    } }));
            }
            else if (cellDataType === modusTable_constants.COLUMN_DEF_DATATYPE_BADGE) {
                return (index.h(ModusTableCellBadgeElement, { badge: cellValue, onBadgeClick: () => {
                        this.cellEl.focus();
                    } }));
            }
            else {
        ...
        return (index.h("div", { class: classes }, this.hasRowsExpandable && index.h(ModusTableCellExpandIcons, { row: row }), index.h("span", { class: wrap ? 'wrap-text' : 'truncate-text' }, renderCell())));
    }
    createErrorTooltip() {
        if (!this.errorTooltip) {
            ...
            this.popperInstance = popper.createPopper(this.cellEl, this.errorTooltip, {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'f83e4a3516e7e09c5ac025e612e16444e9a124dc' }, this.editMode ? (index.h("modus-table-cell-editor", { "data-type": this.cell.column.columnDef[modusTable_constants.COLUMN_DEF_DATATYPE_KEY], value: this.cell.getValue(), type: this.getEditorType(), args: this.getEditorArgs(), valueChange: (newVal) => this.handleCellEditorValueChange(newVal, valueString), keyDown: (event, newVal) => this.handleCellEditorKeyDown(event, newVal, valueString), inputValueChangeHandler: (newVal) => this.handleCellEditorOnInputChange(newVal, valueString) })) : (this.renderCellValue())));
    }
...
const ModusTableFillerColumn = class {
    ...
    render() {
        return (index.h("table", { key: '8c5d31136767a6da879b04edca26d4da23e667a5', id: "table-filler-column", class: { 'cell-borderless': this.cellBorderless, 'd-none': !this.showFillerTable }, ref: (el) => (this.fillerTableRef = el) }, index.h("thead", { key: '458db94209103de571922b9b0bb05146c8033f45' }, index.h("tr", { key: '560d80b509a9bc83f851e972d7d77f68bebfa085' }, index.h("th", { key: '760ed56de2d8d909e5e2e17b25b56148a85537ae' }))), index.h("tbody", { key: 'fc01fb5caf8078b4989268ddb2b56a27820e3d06' }, index.h("tr", { key: '1dda921e8ce27b3538ef858829c5215329319122' }, index.h("td", { key: '2b9208f58aa2f45dac97186d33433ce4e450382e' }))), this.summaryRow && (index.h("tfoot", null, index.h("tr", { class: "summary-row" }, index.h("td", null))))));
    }
};
...
const ModusTableRowActionsCell = class {
    ...
    render() {
        return (index.h(index.Host, { key: '99e83886c3f3c7adb4558b0ca6b4f67ad5b66763' }, index.h("modus-table-row-actions", { key: 'a40fb8324cd122292b9806cd66e28ff747c17b24', row: this.row, context: this.context })));
    }
    get el() { return index.getElement(this); }
};
...
const ModusTableRowActionsMenu = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, this.isMenuOpen && (index.h("div", { style: Object.assign({}, style), class: "row-actions-menu" }, index.h("modus-list", { class: "hydrated" }, this.overFlowMenu.map(({ label, id, icon, iconColor, isDisabled = () => false }) => {
            var _a;
            const disabled = isDisabled((_a = this.tableRow) === null || _a === void 0 ? void 0 : _a.original);
            return (index.h("modus-list-item", { style: { maxWidth: '155px' }, leftIcon: icon, iconColor: iconColor, disabled: disabled, onItemClick: () => this.handleListItemClick(id), class: "hydrated row-actions-menu-item", onKeyDown: (e) => this.handleListItemKeydown(e), tabindex: disabled ? -1 : 0 }, label));
        }))))));
    }
...
const ModusTablePanel = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'd8b9a5275dc4e77bdacb4958d1cdd4079872302c' }, index.h("div", { key: '5f5d87db6a9cd8dd774e055396fd12679d8c534e', class: "table-toolbar" }, index.h("div", { key: '432d5965c73c369d63e55f81ce300dc9c52caee6', class: "section" }, index.h("slot", { key: '11786f71e2d0a2ac80bbf56a5e3de148a57de865', name: "group-left" })), index.h("div", { key: '2b515ed9f8869b8c16ef478deb87fb5f629182cf', class: "section" }, index.h("slot", { key: 'ea1db6297b8534aa58ed9c5a5793b810ff7b8dc3', name: "group-right" }), index.h("modus-table-dropdown-menu", { context: this.context })))));
    }
};
ModusTablePanel.style = ModusTableToolbarStyle0;

exports.modus_pagination = ModusPagination;
exports.modus_table_cell_main = ModusTableCellMain;
exports.modus_table_filler_column = ModusTableFillerColumn;
exports.modus_table_row_actions_cell = ModusTableRowActionsCell;
exports.modus_table_row_actions_menu = ModusTableRowActionsMenu;
