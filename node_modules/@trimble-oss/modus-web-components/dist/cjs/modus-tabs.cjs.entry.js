...
const ModusTabs = class {
    ...
    renderIconWithText(label, leftIcon, rightIcon) {
        return (index.h(index.Fragment, null, leftIcon && (index.h("span", { class: "icon left-icon" }, index.h(ModusIconMap.ModusIconMap, { icon: leftIcon, size: this.size === 'small' ? '16' : '24' }))), index.h("span", { class: "label" }, label), rightIcon && (index.h("span", { class: "icon right-icon" }, index.h(ModusIconMap.ModusIconMap, { icon: rightIcon, size: this.size === 'small' ? '16' : '24' })))));
    }
    renderIconOnly(iconOnly) {
        return (index.h("span", { class: "icon" }, index.h(ModusIconMap.ModusIconMap, { icon: iconOnly, size: this.size === 'small' ? '16' : '24' })));
    }
    render() {
        const tabs = this.tabs.map((tab) => {
            return (index.h("button", { id: `${tab.id}`, class: `tab ${tab.active ? 'active' : ''} ${this.classBySize.get(this.size)} ${this.fullWidth ? 'resizable' : ''} `, onClick: () => this.handleTabChange(tab.id), onKeyDown: (event) => this.handleKeyDown(event, tab.id) }, tab.iconOnly
                ? this.renderIconOnly(tab.iconOnly)
                : this.renderIconWithText(tab.label, tab.leftIcon, tab.rightIcon)));
        });
        return (index.h("div", { "aria-label": this.ariaLabel || undefined, class: `modus-tabs ${this.classBySize.get(this.size)}` }, tabs));
    }
};
ModusTabs.style = ModusTabsStyle0;
