...
const ModusModal = class {
    ...
    renderModal() {
        return (index.h("div", { class: "content", ref: (el) => (this.modalContentRef = el), onMouseDown: () => this.handleModalContentMouseDown() }, index.h(FocusWrap, { id: "startTrap", ref: (el) => (this.startTrapRef = el), onFocus: () => { var _a; return (_a = this.focusWrapping) === null || _a === void 0 ? void 0 : _a.onStartWrapFocus(); } }), this.renderModalHeader(), index.h("div", { class: "body", ref: (el) => (this.modalBodyRef = el) }, index.h("slot", null)), this.renderModalFooter(), index.h(FocusWrap, { id: "endTrap", onFocus: () => { var _a; return (_a = this.focusWrapping) === null || _a === void 0 ? void 0 : _a.onEndWrapFocus(); } })));
    }
    renderModalHeader() {
        return (index.h("header", { class: { scrollable: this.isContentScrollable } }, this.headerText, index.h("div", { class: "header-buttons" }, this.showFullscreenToggle && (index.h("div", { role: "button", tabindex: 0, "aria-label": this.fullscreen ? 'Collapse' : 'Expand', onClick: () => this.toggleFullscreen(), onKeyDown: (event) => this.handleEnterKeydown(event, () => this.toggleFullscreen()) }, this.fullscreen ? index.h(IconExpand.IconCollapse, { size: "24" }) : index.h(IconExpand.IconExpand, { size: "24" }))), index.h("div", { role: "button", tabindex: 0, "aria-label": "Close", onClick: () => this.close(), onKeyDown: (event) => this.handleEnterKeydown(event, () => this.close()) }, index.h(iconClose.IconClose, { size: "24" })))));
    }
    renderModalFooter() {
        return (index.h(index.Fragment, null, index.h("footer", { class: {
                'has-buttons': Boolean(this.primaryButtonText || this.secondaryButtonText),
                scrollable: this.isContentScrollable,
            } }, this.secondaryButtonText && (index.h("modus-button", { disabled: this.secondaryButtonDisabled, "button-style": "outline", color: "secondary", ariaLabel: this.secondaryButtonAriaLabel, onButtonClick: () => this.handleSecondaryClick() }, this.secondaryButtonText)), this.primaryButtonText && (index.h("modus-button", { disabled: this.primaryButtonDisabled, color: "primary", ariaLabel: this.primaryButtonAriaLabel, onButtonClick: () => this.handlePrimaryClick() }, this.primaryButtonText)), index.h("slot", { name: "footerContent" }))));
    }
    render() {
        return (index.h("div", { key: '763cdb0f034096b6ba74cf9041db50350fc50d21', "aria-hidden": this.visible ? undefined : 'true', "aria-label": this.visible ? this.ariaLabel || undefined : undefined, "aria-modal": this.visible ? 'true' : undefined, class: `modus-modal ${this.fullscreen ? 'fullscreen' : ''} overlay ${this.visible ? 'visible' : 'hidden'}`, onClick: (event) => this.handleOverlayClick(event), role: this.visible ? 'dialog' : undefined, style: { zIndex: this.zIndex } }, this.renderModal()));
    }
