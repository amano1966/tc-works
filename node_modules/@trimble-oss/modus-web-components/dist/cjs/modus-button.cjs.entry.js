...
const ModusButton = class {
    constructor(hostRef) {
        ...
        this.handleClickOutside = (event) => {
            if (this.buttonRef && !this.buttonRef.contains(event.target) && this.isCriticalAction()) {
    ...
    handleProgressAnimation() {
        this.pressed = true;
        if (this.isCriticalAction()) {
            this.progressState = Object.assign(Object.assign({}, this.progressState), { progressClass: 'progress', startTime: Date.now() });
    ...
    handleReverseAnimation() {
        ...
        this.progressState.resetTimeout = setTimeout(() => {
            this.progressState = Object.assign(Object.assign({}, this.progressState), { progressWidth: 0, progressClass: '', emitted: false });
            this.buttonRef.classList.remove('reverse');
    ...
    handleKeyup() {
        if (this.keyProgressState.keyDownActive) {
            ...
            this.progressState = Object.assign(Object.assign({}, this.progressState), { progressWidth: Math.min((elapsedTime / this.progressState.animationDuration) * 100, 100), progressClass: 'reverse' });
            this.handleReverseAnimation();
    ...
    renderIconWithText() {
        return (index.h(index.Fragment, null, this.leftIcon && (index.h("span", { class: "icon left-icon" }, index.h(ModusIconMap.ModusIconMap, { icon: this.leftIcon }))), index.h("span", { class: "label" }, index.h("slot", null)), this.rightIcon && !this.showCaret && (index.h("span", { class: "icon right-icon" }, index.h(ModusIconMap.ModusIconMap, { icon: this.rightIcon })))));
    }
    renderIconOnly() {
        return (index.h("span", { class: "icon" }, index.h(ModusIconMap.ModusIconMap, { icon: this.iconOnly })));
    }
    isCriticalAction() {
    ...
    render() {
        ...
        return (index.h("button", { key: 'bde558a95305b44f59be0e4b850d1392f31d762d', "aria-disabled": this.ariaDisabled ? this.ariaDisabled : this.disabled ? 'true' : undefined, "aria-label": this.isCriticalAction()
                ? 'This is a critical action button. Press and hold to confirm.'
                : this.ariaLabel || undefined, "aria-pressed": this.pressed ? 'true' : undefined, class: className, disabled: this.disabled, onClick: () => {
            ...
            }, style: this.isCriticalAction() ? { '--progress-width': `${this.progressState.progressWidth}%` } : {}, onKeyDown: () => (this.pressed = true), onKeyUp: () => (this.pressed = false), onMouseDown: () => this.handleProgressAnimation(), onMouseUp: () => this.handleMouseUp(), onTouchStart: () => this.handleTouchStart(), onTouchEnd: () => this.handleMouseUp(), onTouchCancel: () => this.handleTouchCancel(), ref: (el) => (this.buttonRef = el), type: this.type }, this.iconOnly ? this.renderIconOnly() : this.renderIconWithText(), this.showCaret && index.h(ModusIconMap.ModusIconMap, { size: "24", icon: "caret_down" }), this.isCriticalAction() && (index.h("span", { class: "progress-live-region", "aria-live": "polite", "aria-atomic": "true", role: "status" }, this.progressState.progressWidth > 0 && `${Math.floor(this.progressState.progressWidth)}% complete`))));
    }
    get el() { return index.getElement(this); }
};
ModusButton.style = ModusButtonStyle0;
